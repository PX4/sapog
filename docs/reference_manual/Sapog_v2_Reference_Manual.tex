...
Motor control is performed by means of two standard message types that Sapog subscribes to.
...
Sapog selects a single command from the array at the index specified by the configuration
...
does not prioritize between the supported types of messages
...
minimal response latency and high throughput.
...
and 10 hertz if the motor is starting or running.
...
\subsection{Data type index}

\subsubsection{Broadcast messages}

\subsubsection{Subscribed messages}

\subsubsection{Provided services}

\subsubsection{Invoked services}
