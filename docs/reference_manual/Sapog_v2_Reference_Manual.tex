...
Having started, the bootloader checks if there is a valid application image that can be started.
If there is one, the bootloader measures a 5 second timeout since the point of its initialization,
and when the timeout has expired, the bootloader starts the application, unless an external
...
if the application had already known the bit rate of the bus, it will pass it to the bootloader.
